...
function Login() {
  ...
  const handleCloseGlobalError = () =>
    setErrors((prev) => ({ ...prev, globalErr: "" }));
  const handleCloseSuccess = () => setSuccessMsg("");

  const inputClass = (error, value) =>
    `peer block w-full rounded-lg px-2.5 pb-2.5 pt-5 text-sm ${
  ...
  return (
    ...
    >
      <div className={`max-w-lg w-full ${cardClass} shadow-lg rounded-xl p-6`}>
          ...
          <div dir={direction} className="relative">
            ...
            {errors.emailErr ? (
              ...
              <p className="mt-2 text-xs text-green-600">
                <span className="font-medium">{t("Well done!")}</span>{" "}
                {t("Looks good.")}
              </p>
            ) : null}
          </div>
